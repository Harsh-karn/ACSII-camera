...
const getClient = () => {
  ...
  if (!apiKey || apiKey === 'YOUR_API_KEY_HERE' || apiKey === '') {
    throw new Error("CONFIG_ERROR: Please set your GEMINI_API_KEY in the .env file.");
  }
...
export const analyzeImage = async (base64Image: string): Promise<AnalysisResult> => {
  try {
    ...
    console.log("Starting analysis with model:", 'gemini-3-flash-preview');
    ...
    return {
      description: isConfigError 
        ? "CONFIG ERROR: ACCESS DENIED. PLEASE INITIALIZE AUTHENTICATION KEY IN .env FILE."
        : "ANALYSIS FAILED. UNABLE TO PROCESS VISUAL DATA. RETRY INITIATED.",
      threatLevel: "ERROR",
      tags: isConfigError ? ["AUTH_REQUIRED", "OFFLINE"] : ["ERROR", "NO_DATA"]
    };
  }
};