...
import { ScanEye, Camera, User } from 'lucide-react';
...
export const AsciiCanvas: React.FC<AsciiCanvasProps> = ({ options, onCapture }) => {
  ...
  const isProcessingMask = useRef(false);
  ...
  useEffect(() => {
    const renderLoop = () => {
      ...
      // --- SEGMENTATION START ---
      if (options.subjectOnly && net && !isProcessingMask.current) {
        isProcessingMask.current = true;
        net.segmentPerson(video, {
          flipHorizontal: true,
          internalResolution: 'high',
          segmentationThreshold: 0.8
        }).then(segmentation => {
          // Rescale segmentation data to our ASCII grid (cols x rows)
          // Simple nearest neighbor or just use the raw data if we can
          maskRef.current = segmentation.data;
          isProcessingMask.current = false;
        }).catch(() => {
          isProcessingMask.current = false;
        });
      }
      ...
      if (options.colorMode === 'color') {
          ...
          for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
                ...
                // Apply Mask
                if (options.subjectOnly && maskRef.current) {
                    // Segmentation data is [videoWidth * videoHeight]
                    // We need to map (x, y) to (videoX, videoY)
                    const vx = Math.floor((x / cols) * video.videoWidth);
                    const vy = Math.floor((y / rows) * video.videoHeight);
                    const vIdx = vy * video.videoWidth + vx;
                    if (maskRef.current[vIdx] === 0) brightness = 0;
                }
