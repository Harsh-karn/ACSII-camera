...
import { Sliders, Monitor, Type, Palette, User } from 'lucide-react';
    ...
    <div className="absolute bottom-0 w-full bg-black/80 border-t border-green-900/50 backdrop-blur-sm p-4 z-30 transition-all duration-300">
      <div className="max-w-6xl mx-auto flex flex-wrap gap-6 justify-center items-center text-green-500 text-xs font-mono">
        ...
        <div className="flex flex-col gap-2">
            ...
            <button
                onClick={() => handleModeChange('subjectOnly', !options.subjectOnly)}
                className={`px-3 py-1 border ${options.subjectOnly ? 'bg-green-500 text-black border-green-500' : 'bg-transparent border-green-800 text-green-700 hover:border-green-500'} text-[10px] uppercase transition-colors flex items-center gap-1`}
            >
                {options.subjectOnly ? 'SUBJECT' : 'FULL FRAME'}
            </button>
        </div>
